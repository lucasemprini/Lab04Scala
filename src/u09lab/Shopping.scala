...
case class Cart(items: Map[Product, ItemDetails] = Map()){
  def add(item: Item) = {
    Cart(items + (item.product -> items.get(item.product).map(i =>
      i.copy(qty = i.qty + item.details.qty,
      price = Price(i.price.value+item.details.price.value))).getOrElse(item.details)))
  }
  def totalCost: Double = items.values.foldRight(0.0){ case (item, acc) => item.price.value + acc }
}
